...
void VideoPlayer_init(void) {
	...
	video_quality_selector_view = (new SelectorView(0, 0, 320, 35, false))
	                                  ->set_texts({(std::function<std::string()>)[]() { return LOCALIZED(OFF); }}, 0)
	            ...
	            {(new TextView(SMALL_MARGIN, 0, 320, DEFAULT_FONT_INTERVAL * 8))
	             ...
	             (new CustomView(0, 0, 320, 160))->set_draw([](const CustomView &view) {
		             ...
		             Draw("Deadline : " + std::to_string(vid_frametime).substr(0, 5) + "ms", 0, y + 100, 0.4, 0.4,
		                  0xFFFFFF00);
		             Draw("Video decode : " + std::to_string(vid_video_time).substr(0, 5) + "ms", 0, y + 110, 0.4, 0.4,
		                  DEF_DRAW_RED);
		             Draw("Audio decode : " + std::to_string(vid_audio_time).substr(0, 5) + "ms", 0, y + 120, 0.4, 0.4,
		                  DEF_DRAW_RED);
		             // Draw("Data copy 0 : " + std::to_string(vid_copy_time[0]).substr(0, 5) + "ms", 160, 120, 0.4,
		             // 0.4, DEF_DRAW_BLUE);
		             Draw("Color convert : " + std::to_string(vid_convert_time).substr(0, 5) + "ms", 160, y + 110, 0.4,
		                  0.4, DEF_DRAW_BLUE);
		             Draw("Data copy 1 : " + std::to_string(vid_copy_time[1]).substr(0, 5) + "ms", 160, y + 120, 0.4,
		                  0.4, DEF_DRAW_BLUE);
		             Draw("Thread 0 : " + std::to_string(vid_time[0][319]).substr(0, 6) + "ms", 0, y + 130, 0.5, 0.5,
		                  DEF_DRAW_RED);
		             Draw("Thread 1 : " + std::to_string(vid_time[1][319]).substr(0, 6) + "ms", 160, y + 130, 0.5, 0.5,
		                  DEF_DRAW_BLUE);
		             Draw("Zoom : x" + std::to_string(vid_zoom).substr(0, 5) + " X : " + std::to_string((int)vid_x) +
		                      " Y : " + std::to_string((int)vid_y),
	            ...
	            {(new EmptyView(0, 0, 320, 0)), // margin at the top(using ScrollView's auto margin)
	             ...
	             (new BarView(0, 0, 260, 30))                     // preamp
	                 ...
	                 ->set_title([](const BarView &view) {
		                 return LOCALIZED(PREAMP) + " : " +
		                        std::to_string((int)std::round(std::exp(view.get_value()) * 100)) + "%";
	                 })
...
static PostView *comment_to_view(const YouTubeVideoDetail::Comment &comment, int comment_index) {
	...
	while (itr != cur_content.end()) {
		...
		auto tmp = truncate_str(std::string(itr, next_itr), COMMENT_MAX_WIDTH, COMMENT_MAX_LINE_NUM - cur_lines.size(),
		                        0.5, 0.5);
	...
	if (is_to_display) {
			    ...
			    (new HorizontalListView(0, 0, ICON_SIZE)) // author
			        ->set_views(
			            {(new EmptyView(0, 0, SMALL_MARGIN, ICON_SIZE)), new_main_icon_view,
			             ...
			             (new VerticalListView(0, 0, 320 - SMALL_MARGIN * 3 - ICON_SIZE - SUBSCRIBE_BUTTON_WIDTH))
			                 ->set_views(
			                     {(new EmptyView(0, 0, 320 - SMALL_MARGIN * 3 - ICON_SIZE - SUBSCRIBE_BUTTON_WIDTH,
			                                     ICON_SIZE * 0.1)),
			                      (new TextView(0, 0, 320 - SMALL_MARGIN * 3 - ICON_SIZE - SUBSCRIBE_BUTTON_WIDTH,
			             ...
			             (new CustomView(0, 0, SUBSCRIBE_BUTTON_WIDTH, ICON_SIZE))
			                 ->set_draw([author_id](const CustomView &view) {
				                 ...
				                 std::string button_text = is_subscribed ? LOCALIZED(SUBSCRIBED) : LOCALIZED(SUBSCRIBE);
				                 ...
				                 Draw_x_centered(button_text, view.x0, view.x0 + SUBSCRIBE_BUTTON_WIDTH, button_y + 4,
				                                 0.5, 0.5, 0xFF000000);
			                 })
		...
		View *exit_button_view =
		    (new HorizontalListView(0, 0, exit_button_height))
		        ->set_views(
		            {(new EmptyView(0, 0, 320 - 100, exit_button_height))->set_get_background_color([](const View &) {
		...
		caption_left_view->views.push_back(
		    (new TextView(0, 0, 160, DEFAULT_FONT_INTERVAL * 1.2))
		        ...
		        ->set_get_background_color([](const View &) { return DEFAULT_BACK_COLOR; }));
		caption_left_view->views.push_back(
		    (new RuleView(0, 0, 160, 3))->set_get_background_color([](const View &) { return DEFAULT_BACK_COLOR; }));
			    ...
			    (new TextView(0, 0, 160, DEFAULT_FONT_INTERVAL))
			        ...
			        ->set_get_background_color([cur_lang](const View &view) {
				        if (cur_lang.id == selected_base_lang) {
					        ...
					        int darkness =
					            std::min<int>(0xFF, 0xD0 + 0x30 * (1 - std::min(1.0, 0.15 * view.view_holding_time)));
					        if (var_night_mode) {
		...
		caption_right_view->views.push_back(
		    (new SelectorView(0, 0, 160, DEFAULT_FONT_INTERVAL * 2.5, false))
		        ...
		        ->set_on_change([translation_languages_selector_view](const SelectorView &view) {
			        translation_languages_selector_view->set_is_visible(view.selected_button)
			            ->set_is_touchable(view.selected_button);
		        })
		        ->set_get_background_color([](const View &) { return DEFAULT_BACK_COLOR; }));
		...
		if (tmp_video_info.playlist.selected_index >= 0 &&
			...
			if (playlist_view_scroll < selected_y - (CONTENT_Y_HIGH - PLAYLIST_TOP_HEIGHT)) {
				playlist_view_scroll =
				    selected_y - (CONTENT_Y_HIGH - PLAYLIST_TOP_HEIGHT) + SUGGESTIONS_VERTICAL_INTERVAL;
			}
		...
		playlist_view_scroll = std::min<double>(playlist_view_scroll,
		                                        tmp_video_info.playlist.videos.size() * SUGGESTIONS_VERTICAL_INTERVAL -
		...
		if (cur_video_info.playlist.videos.size()) {
			...
			main_view
			    ->set_views(
			        {main_tab_view, suggestion_tab_view, comment_tab_view, captions_tab_view, playback_tab_view},
			        TAB_GENERAL)
			    ->set_tab_texts<std::function<std::string()>>(
			        {[]() { return LOCALIZED(GENERAL); }, []() { return LOCALIZED(SUGGESTIONS); },
			         []() { return LOCALIZED(COMMENTS); }, []() { return LOCALIZED(CAPTIONS); },
	        ...
	        ->set_views({(new SelectorView(0, 0, 320 - top_button_width, top_button_height, false))
	                     ...
	                     (new TextView(0, 0, top_button_width, top_button_height))
	                         ...
	                         ->set_on_view_released(
	                             [](View &view) { captions_tab_view->set_subview(caption_language_select_view); })
	                         ->set_background_color(COLOR_GRAY(0x80))}));
...
void video_draw_playing_bar() {
	...
	bar_x_l = 30;
	bar_x_r =
	    320 - std::max(time_str0_w, time_str1_w) - TIME_STR_RIGHT_MARGIN - TIME_STR_LEFT_MARGIN - MAXIMIZE_ICON_WIDTH;
...
static void decode_thread(void *arg) {
	...
	while (vid_thread_run) {
		if (vid_play_request || vid_change_video_request) {
			...
			if (audio_only_mode) {
				result =
				    network_decoder.init(playing_video_info.audio_stream_url, stream_downloader,
				...
				result = network_decoder.init(
				    ...
				    stream_downloader, playing_video_info.is_livestream ? playing_video_info.stream_fragment_len : -1,
				...
				result =
				    network_decoder.init(playing_video_info.both_stream_url, stream_downloader,
				                         playing_video_info.is_livestream ? playing_video_info.stream_fragment_len : -1,
				                         playing_video_info.needs_timestamp_adjusting(), var_is_new3ds);
			} else {
				result.code = -1;
					...
					if (!eof_reached) { // the first time it reaches EOF
						...
						if (var_loop_mode == false) {
							if ((var_autoplay_level == 2 && playing_video_info.has_next_video()) ||
								...
								send_change_video_request_wo_lock(playing_video_info.get_next_video().url, true, false,
								                                  false);
								break_flag = true;
							}
				...
				if (type == NetworkMultipleDecoder::PacketType::AUDIO) {
					...
					if (vid_play_request && !vid_seek_request && !vid_change_video_request) {
						if (result.code != 0 && result.code != DEF_ERR_NEED_MORE_INPUT) {
							logger.error(DEF_SAPP0_DECODE_THREAD_STR,
							             "Util_video_decoder_decode()..." + result.string + result.error_description,
							             result.code);
						}
					}
			...
			while (vid_play_request && !vid_seek_request && !vid_change_video_request) {
				...
				if (result.code == 0) {
					...
					if (!video_skip_drawing) {
						...
						result = Draw_set_texture_data(&vid_image[texture_index_head], video, vid_width, vid_height_org,
						                               1024, 1024, GPU_RGB565);
						if (result.code != 0) {
							logger.error(DEF_SAPP0_CONVERT_THREAD_STR,
